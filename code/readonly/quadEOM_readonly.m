% written by Daniel Mellinger July 1, 2011
%
% For details of the controller see the paper:
%
% Daniel Mellinger, Nathan Michael, and Vijay Kumar. Trajectory Generation and
% Control for Precise Aggressive Maneuvers with Quadrotors. Int. Symposium on
% Experimental Robotics, Dec 2010.
%
...
function sdot = quadEOM_readonly(t, true_state, F, M, Fd)
    ...
    %
    % % Prop speeds omega2 omega2 = params.omega2_FM*[F;M]; % enforce a max &
    % min prop speed if(~isempty(params.maxomega))
    %     omega2 = min(omega2,params.maxomega^2*ones(4,1));
    % end if(~isempty(params.minomega))
    %     omega2 = max(omega2,params.minomega^2*ones(4,1));
    % end % Convert back to force & moment [FM] = params.FM_omega2*omega2; %
    % Re-assemble Control input F    = FM(1); M    = FM(2:4);

    % Assign states x = true_state(1); y = true_state(2); z = true_state(3);
    xdot = true_state(4);
    ydot = true_state(5);
    zdot = true_state(6);
    qW = true_state(7);
